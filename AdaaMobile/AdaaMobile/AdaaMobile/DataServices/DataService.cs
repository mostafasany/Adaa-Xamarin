...
{
    //    // Attendance
    //1.       https://adaamobile.adaa.abudhabi.ae/proxyservice/proxy?server=adaamobile&url=/?funcname=getClients&langid=Eng&userToken=2016070523215708204 

    //// SharePoint
    //2.       https://adaamobile.adaa.abudhabi.ae/proxyservice/proxy?server=imach&url=/?funcname=getEquipments&langid=eng&userToken=2016030614303200503&empID=2013001 

    //// Timesheet
    //3.       https://adaamobile.adaa.abudhabi.ae/proxyservice/proxy?server=timesheet&url=/getWeeksPerYear?year=2016&lang=en 

    ////Service Desk
    //4.       https://adaamobile.adaa.abudhabi.ae/proxyservice/proxy?server=servicedesk&url=GetDomainName
    ...
    public class DataService : IDataService
    {
        ...
        private const string ServiceDiskBaseUrl = "https://adaamobile.adaa.abudhabi.ae/proxyservice/proxy?server=servicedesk&url=";
        ...
        private const string TimeSheetBaseUrl = "https://adaamobile.adaa.abudhabi.ae/proxyservice/proxy?server=timesheet&url=/";
        ...
        public async Task<ResponseWrapper<string>> NewServiceRequest(NewServiceRequest requestObject, CancellationToken? token = default(CancellationToken?))
        {
            var request = _requestFactory();
            request.ResultContentType = ContentType.Json;
            request.RequestUrl = ServiceDiskBaseUrl + string.Format("SaveMultiServiceRequest");
            var arr = new List<NewServiceRequest>();
            arr.Add(requestObject);
            var result = JsonConvert.SerializeObject(arr.ToArray());
            var stringContent = new StringContent(result, new UTF8Encoding(), JSONContentType);
